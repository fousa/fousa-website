...
### Collections
- **Employer**: every job and freelance entity. Drives the career timeline on the about page. Also referenced by Project via the `employer` field — shared document so career and project data stay in sync. Uses `startDate`/`endDate` (date, month precision) for tenure; `pinned` (boolean) lifts the freelance umbrella (Fousa) above the chronological list. Engagement types: freelance, full-time, owner, internship, holiday, education.
